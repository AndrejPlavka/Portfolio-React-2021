...
export function useTaskLocalStorage<T>(key: string, initialValue: T) {
  ...
  const [todos, setTodosLocally] = useState<T>(() => {
    try {
  ...
  const setTodos = (value: T | ((val: T) => T)) => {
    try {
      // Allow value to be a function so we have same API as useState
      const valueToStore = value instanceof Function ? value(todos) : value;
      // Save state
      setTodosLocally(valueToStore);
